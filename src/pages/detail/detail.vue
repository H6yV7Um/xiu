<template>
	<div id="app">
		<mt-header :title="title">
			<mt-button slot="left" @click="toBack">
		...
		<div class="">
			...
			<div>
				<mt-navbar v-model="selected">
					<mt-tab-item id="1">互动区</mt-tab-item>
					<mt-tab-item id="2">详情</mt-tab-item>
					<mt-tab-item id="3">相关课程</mt-tab-item>
				</mt-navbar>
				...
				<mt-tab-container v-model="selected">
						...
						<div>
							...
							<div class="text">
								<fieldset class="fieldset">
									<legend class="legend">
										<img :src="lecturerInfo.photoUrl" alt="">
										<span class="name">{{ lecturerInfo.lecturerName }}</span>
									</legend>
									<p>
										{{ lecturerInfo.content }}
									</p>
								</fieldset>
							</div>
							<div class="border"></div>
							<div class="c-detail">
								<p class="c-detail-title">
									课程详情
								</p>
								<div class="content">
									<div style="text-align: center"><strong>{{ title }}</strong></div>
									<dl>
										<!-- <dt><strong>课程大纲</strong></dt> -->
										<dd v-html="lecturerInfo.topicContent"></dd>
									</dl>
									<!-- <div class="c-detail-text">
										详细介绍详细介绍详细介绍详细介绍详细介绍详细介绍详细介绍详细介绍详细介绍详细介绍详细介绍详细介绍详细介绍
									</div> -->
								</div>
							</div>
						</div>
					...
					<mt-tab-container-item id="3">
						<div class="list"><list @toDetail="toDetail"></list></div>
					</mt-tab-container-item>
				</mt-tab-container>
			</div>
		</div>
...
import { Popup, Indicator, Header, Button, Navbar, TabItem, TabContainer, TabContainerItem, MessageBox } from 'mint-ui'
...
export default {
	...
	created () {
		this.title = utils.getUrlParam('title')
		this.queryLecture()
		this.queryContent()
	},
	mounted () {
		this.joinMetting()
		const videoUrl = utils.getUrlParam('video_url')
		const coverpic = utils.getUrlParam('cover_url')
		const isLive = utils.getUrlParam('is_live')
		// MessageBox('提示', isLive)
		/* eslint-disable no-new */
		new TcPlayer('id_video_container', {
			'm3u8': videoUrl,
			'autoplay': false,
			'coverpic': {
				'src': coverpic,
				'style': 'cover' // cover,stretch
			},
			'width': '480',
			'height': '210',
			'live': isLive === '1' ? true : false
		})
		this.metting()
	},
	methods: {
		...
		queryLecture () {
			let paras = `userId=${utils.getUrlParam('user_id')}`
			paras = Common.setQueryString(paras)
			let url = process.env.API_URL18091 + '/topicUserController.do?topic_user_detail&' + paras + '&token=' + localStorage.getItem('$token')
			this._query(url, data => {
				const info = data.result_map.result_map
				this.lecturerInfo.lecturerName = info.lecturerName
				this.lecturerInfo.content = info.content
				this.lecturerInfo.photoUrl = info.photoUrl
			})
		},
		queryContent () {
			let paras = `topic_id=${utils.getUrlParam('topic_id')}`
			paras = Common.setQueryString(paras)
			let url = process.env.API_URL18091 + '/topicController.do?topic_content_query&' + paras + '&token=' + localStorage.getItem('$token')
			this._query(url, data => {
				if (data.result_map.result_map) {
					this.lecturerInfo.topicContent = data.result_map.result_map.topicContent
				} else {
					this.lecturerInfo.topicContent = '没有数据'
				}
			})
		},
		queryChartRoomId (chartRoomId) {
			let paras = `chat_room_id=${chartRoomId}`
			paras = Common.setQueryString(paras)
			let url = 'http://101.132.181.5:18091/chatRoomController.do?chat_room_sign&' + paras + '&token=' + localStorage.getItem('$token')
			this._query(url, data => {
		...
		joinMetting () {
			...
			/* eslint-disable */
			alert = MessageBox
			var mobile = localStorage.getItem('$mobile_tel') || '12345678910';
			document.getElementById('onSendMsg').onclick = function () {
				if (localStorage.getItem('$token')) {
					// 判断token是否有效
					let paras = ''
					paras = Common.setQueryString(paras)
					const url = process.env.API_URL18088 + '/accountController.do?account_info_query&' + paras + '&token=' + localStorage.getItem('$token')
					Indicator.open()
					$.ajax({
						url: url,
						type: 'get',
						...
						success: function (data) {
							if (data.login_flag === '1') {
								mobile = localStorage.getItem('$mobile_tel')
								onSendMsg()
							} else {
								MessageBox.confirm('请登录后发言').then(action => {
			...
			document.getElementById('showEmotionDialog').onclick = function () {
				if (localStorage.getItem('$token')) {
					// 判断token是否有效
					let paras = ''
					paras = Common.setQueryString(paras)
			...
			var accountMode = 0;
			var sdkAppID = 1400071639;
			var accountType = 23149;
			var avChatRoomId = '@TGS#3UJPZCDFX';
			...
			var selSess = null;
			var selSessHeadUrl = './assets/img/2017.jpg';
			//当前用户身份
			var loginInfo = {
					'sdkAppID': sdkAppID,
					'appIDAt3rd': sdkAppID,
					...
					'identifier': mobile,
					'identifierNick': mobile.substr(0, 3) + '****' + mobile.substr(7),
					'userSig': null,
			...
			// $('#login_dialog').show();
			var paras = 'chat_room_id=' + getUrlParam('chat_room_id');
			paras = Common.setQueryString(paras);
			var url = process.env.API_URL18091 + '/chatRoomController.do?chat_room_detail&' + paras;
			$.ajax({
				url: url,
				type: 'get',
				dataType: 'jsonp',
				...
				success: function (data) {
					if (data.success_type === '1') {
						data = data.result_map.result_map;
						sdkAppID = parseInt(data.chatRoomId)
						loginInfo.sdkAppID = sdkAppID
						loginInfo.appIDAt3rd = sdkAppID
						avChatRoomId = data.avChatRoomId
						selToID = avChatRoomId
						accountType = parseInt(data.accountType)
						loginInfo.accountType = accountType
						var paras = 'chatRoom=' + sdkAppID + '&identifier=' + loginInfo.identifier;
						paras = Common.setQueryString(paras);
						var url = process.env.API_URL18091 + '/chatRoomController.do?chat_room_sign&' + paras;
						$.ajax({
							url: url,
							type: 'get',
							dataType: 'jsonp',
			...
			function getGroupList(selToID, cbOk, cbError) {
			...
			function applyJoinBigGroup(groupId) {
				...
				webim.applyJoinBigGroup(
						...
						function(err) {
							const isLive = getUrlParam('is_live')
							if (isLive == '0') {
								getGroupList(avChatRoomId)
							}
						}
				);
			}
			...
			function showMsg(msg) {
				...
				var color = colorList[index]; // userId.substr(0, 3) + '****' + userId.substr(7)
				var newDate = new Date()

				var time = webim.Tool.formatTimeStamp(msg.getTime())
				nickNameSpan.setAttribute('class', 'user-name-' + color);
				fp.innerHTML = webim.Tool.formatText2Html("" + fromAccountNick.substr(0, 3) + '****' +fromAccountNick.substr(7) + "");
				lp.innerHTML = webim.Tool.formatText2Html(time);
				nickNameSpan.appendChild(fp);
				nickNameSpan.appendChild(lp);
				contentSpan = document.createElement("p");
				sessType = msg.getSession().type();
				//获取消息子类型
				//会话类型为群聊时，子类型为：webim.GROUP_MSG_SUB_TYPE
				//会话类型为私聊时，子类型为：webim.C2C_MSG_SUB_TYPE
				...
				switch (subType) {
					...
					case webim.GROUP_MSG_SUB_TYPE.TIP: //群提示消息
						contentSpan.innerHTML = "[消息]" + convertMsgtoHtml(msg);
						break;
					default:
